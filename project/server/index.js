...
const uploadLecture=multer({storage:multer.memoryStorage(),limits:{fileSize:50*1024*1024,files:1}});
...
app.post('/api/lectures/:id/media/:kind',requireUser,uploadLecture.single('file'),wrap(async(req,res)=>{ await owner(req,'lectures',req.params.id);const kind=req.params.kind,file=req.file;if(!file||!['audio','pdf','video'].includes(kind)||kind==='pdf'&&file.mimetype!=='application/pdf'||kind==='audio'&&(!file.mimetype.startsWith('audio/')||file.size>25*1024*1024)||kind==='video'&&!['video/mp4','video/webm'].includes(file.mimetype))throw fail(400,'نوع الملف غير مدعوم أو حجمه أكبر من المسموح.');await query('INSERT INTO media(id,lecture_id,kind,mime,content) VALUES($1,$2,$3,$4,$5) ON CONFLICT(lecture_id,kind) DO UPDATE SET mime=$4,content=$5',[id(),req.params.id,kind,file.mimetype,file.buffer]);res.json({ok:true}); }));
app.get('/api/lectures/:id/media/:kind',requireUser,wrap(async(req,res)=>{ const {rows}=await query('SELECT mime,content FROM media WHERE lecture_id=$1 AND kind=$2',[req.params.id,req.params.kind]);if(!rows.length)throw fail(404,'الملف غير موجود.');const {mime,content}=rows[0];res.type(mime);res.set('Cache-Control','private, no-store');res.set('Accept-Ranges','bytes');res.set('Content-Disposition',`inline; filename="lecture-${req.params.id}.${req.params.kind==='pdf'?'pdf':req.params.kind==='video'?(mime==='video/webm'?'webm':'mp4'):'audio'}"`);const range=req.headers.range;if(range){const match=/^bytes=(\d*)-(\d*)$/.exec(range);if(!match || (!match[1]&&!match[2])){res.status(416).set('Content-Range',`bytes */${content.length}`).end();return;}let start,end;if(!match[1]){const suffix=Number(match[2]);start=Math.max(0,content.length-suffix);end=content.length-1;}else{start=Number(match[1]);end=match[2]?Number(match[2]):content.length-1;}if(!Number.isSafeInteger(start)||!Number.isSafeInteger(end)||start>=content.length||end<start||content.length===0){res.status(416).set('Content-Range',`bytes */${content.length}`).end();return;}end=Math.min(end,content.length-1);res.status(206).set('Content-Range',`bytes ${start}-${end}/${content.length}`).set('Content-Length',String(end-start+1)).send(content.subarray(start,end+1));return;}res.send(content); }));
