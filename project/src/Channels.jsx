  ...
  return <div className="channels-area">
    ...
    {active ? <>
      ...
      {!isTeacher && !joined.includes(active.id) ? <div className="channel-join-required"><p>انضم إلى القناة للاطلاع على منشوراتها.</p><button className="primary-button" onClick={() => onJoin(active.id)}>الانضمام للقناة</button></div> : <>
        ...
        {(active.posts || []).length ? <div className="channel-feed">{active.posts.map(post => <article className="channel-post" key={post.id}><div className="channel-post-top"><div className="channel-post-avatar"><BookOpen size={17} /></div><div><strong>{active.teacherName || 'أستاذ المادة'}</strong><small>{post.date}</small></div>{isTeacher && <button className="channel-delete" aria-label="حذف المنشور" onClick={() => onDeletePost(active.id, post.id)}><Trash2 size={17} /></button>}</div>{post.text && <p>{post.text}</p>}{post.imageUrl && <img src={post.imageUrl} alt={post.text ? `صورة مرفقة بالمنشور: ${post.text.slice(0, 80)}` : 'صورة مرفقة بمنشور الأستاذ'} loading="lazy" />}{post.imageName && !post.imageUrl && <div className="channel-image-missing">هذه الصورة غير متاحة في هذا المتصفح.</div>}</article>)}</div> : <div className="channel-empty"><ImagePlus size={25} /><p>لم يُنشر شيء في هذه القناة بعد.</p></div>}
      </>}
    </> : <>
