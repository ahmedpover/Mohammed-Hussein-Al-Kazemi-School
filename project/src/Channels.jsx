...
export default function Channels({ role, userId, channels, joined, onCreate, onJoin, onLeave, onPost, onDeletePost, onDeleteChannel }) {
  ...
  const mine = isTeacher ? channels.filter(channel => !channel.ownerId || channel.ownerId === userId) : channels.filter(channel => joined.includes(channel.id));

  return <div className="channels-area">
    <div className="channels-intro">
      <div><span className="overline">قنوات المواد</span><h3>{isTeacher ? 'انشر توجيهات المادة في قناتها' : 'تابع توجيهات أساتذتك'}</h3><p>{isTeacher ? 'أنشئ قناة بمعرّف خاص، ثم انشر التعليمات والصور لطلاب المادة.' : 'ابحث عن معرّف القناة الذي يعطيك إياه الأستاذ، ثم انضم إليها.'}</p></div>
    ...
    {active ? <>
      ...
      {!isTeacher && !joined.includes(active.id) ? <div className="channel-join-required"><p>انضم إلى القناة للاطلاع على منشوراتها.</p><button className="primary-button" onClick={() => onJoin(active.id)}>الانضمام للقناة</button></div> : <>
        <div className="channel-detail-actions">{isTeacher ? <button className="text-button danger-text" onClick={async () => { if (await onDeleteChannel(active)) setActiveId(null); }}>حذف القناة <Trash2 size={16} /></button> : <button className="text-button" onClick={() => { onLeave(active.id); setActiveId(null); }}>مغادرة القناة</button>}</div>
        ...
        {(active.posts || []).length ? <div className="channel-feed">{active.posts.map(post => <article className="channel-post" key={post.id}><div className="channel-post-top"><div className="channel-post-avatar"><BookOpen size={17} /></div><div><strong>أستاذ المادة</strong><small>{post.date}</small></div>{isTeacher && <button className="channel-delete" aria-label="حذف المنشور" onClick={() => onDeletePost(active.id, post.id)}><Trash2 size={17} /></button>}</div>{post.text && <p>{post.text}</p>}{post.imageUrl && <img src={post.imageUrl} alt={post.text ? `صورة مرفقة بالمنشور: ${post.text.slice(0, 80)}` : 'صورة مرفقة بمنشور الأستاذ'} loading="lazy" />}{post.imageName && !post.imageUrl && <div className="channel-image-missing">هذه الصورة غير متاحة في هذا المتصفح.</div>}</article>)}</div> : <div className="channel-empty"><ImagePlus size={25} /><p>لم يُنشر شيء في هذه القناة بعد.</p></div>}
      </>}
    </> : <>
      {!isTeacher && <form className="channel-search" onSubmit={event => { event.preventDefault(); setLookedUp(search.trim().replace(/^@/, '').toLowerCase()); }}><label htmlFor="channel-handle-search">البحث بمعرّف القناة</label><div><Hash size={19} /><input id="channel-handle-search" dir="ltr" value={search} onChange={event => { setSearch(event.target.value); setLookedUp(''); }} placeholder="مثال: fiqh_stage1" autoComplete="off" /><button className="primary-button" type="submit"><Search size={17} /> بحث</button></div></form>}
      ...
      <div className="channel-list-head"><h3>{isTeacher ? 'قنوات المواد' : 'قنواتي'}</h3><span>{mine.length} قنوات</span></div>
      {mine.length ? <div className="channel-grid">{mine.map(channel => <ChannelCard key={channel.id} channel={channel} joined={joined.includes(channel.id)} teacher={isTeacher} onOpen={() => setActiveId(channel.id)} />)}</div> : <div className="channel-empty"><Users size={27} /><p>{isTeacher ? 'لم تنشئ قناة بعد. ابدأ بقناة لمادة تدرّسها.' : 'لم تنضم إلى أي قناة بعد. ابحث عن معرّف قناة المادة.'}</p></div>}
    </>}
    {createOpen && <CreateChannel channels={channels} onClose={() => setCreateOpen(false)} onCreate={channel => { onCreate(channel); setCreateOpen(false); setActiveId(channel.id); }} />}
  </div>;
}
...
function CreateChannel({ channels, onClose, onCreate }) {
