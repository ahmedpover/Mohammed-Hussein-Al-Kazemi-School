...
function TeacherManagement({ invites, onSave }) {
  ...
  const open = item => { setEditing(item?.email || null); setEmail(item?.email || ''); setName(item?.name || ''); setSubjects(item?.subjects?.join('، ') || ''); setActive(item?.active ?? true); setInviteCode(''); setError(''); };
