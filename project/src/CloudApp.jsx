...
function LectureForm({ initial, subjects, onSave, onCancel }) {
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  async function submit(e) { e.preventDefault(); setError(''); if(video && (video.size>50*1024*1024 || !['video/mp4','video/webm'].includes(video.type))){setError('اختر فيديو MP4 أو WebM بحجم لا يتجاوز ٥٠ ميغابايت.');return;} setBusy(true); try { await onSave({ title, subject, book, description, audio, video }); } catch (cause) { setError(cause.message || 'تعذر حفظ المحاضرة.'); } finally { setBusy(false); } }
  return <form className="cloud-lecture-form" onSubmit={submit}><label>عنوان المحاضرة<input value={title} onChange={e => setTitle(e.target.value)} required maxLength="100" /></label><label>المادة<select value={subject} onChange={e => setSubject(e.target.value)} required disabled={Boolean(initial)}>{subjects.map(item => <option key={item}>{item}</option>)}</select></label><label>الكتاب<input value={book} onChange={e => setBook(e.target.value)} required maxLength="100" /></label><label>وصف مختصر<textarea value={description} onChange={e => setDescription(e.target.value)} maxLength="500" /></label><div className="cloud-files"><label>تسجيل صوتي <input type="file" accept="audio/*" onChange={e => setAudio(e.target.files[0] || null)} /></label><label>فيديو المحاضرة <input type="file" accept="video/mp4,video/webm" onChange={e => setVideo(e.target.files[0] || null)} /><small>MP4 أو WebM، بحد أقصى ٥٠ ميغابايت.</small></label></div>{error && <p role="alert" className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onCancel}>إلغاء</button><button className="primary-button" disabled={busy || !subjects.length}>{busy ? 'جارٍ رفع المحاضرة…' : 'حفظ المحاضرة'}</button></div></form>;
}

function LectureView({ lecture, onBack }) {
  return <section className="section-block cloud-lecture-view"><button className="back-button" onClick={onBack}><ArrowRight size={18} /> المحاضرات</button><span className="overline">{lecture.subject} · {lecture.book}</span><h2>{lecture.title}</h2><p>{lecture.description}</p><span className="scope-note">{lecture.teacherName} · {lecture.date || formatDate(lecture.createdAt)}</span>{lecture.videoUrl && <video controls playsInline preload="metadata" src={lecture.videoUrl} />}{lecture.audioUrl && <audio controls preload="metadata" src={lecture.audioUrl} />}{lecture.pdfUrl && <a className="outline-button" href={lecture.pdfUrl} target="_blank" rel="noreferrer"><FileText size={18} /> عرض ملف PDF</a>}{!lecture.videoUrl && !lecture.audioUrl && !lecture.pdfUrl && <p className="quiet-empty">لم تُرفق ملفات بهذه المحاضرة.</p>}</section>;
}
